...
%          already corrected.
%     wall_size: size of sncanned wall along one dimension (assumes 
...
%         for time-of-flight delays
    ...
    % Constants
    bin_resolution = 4e-12 * 4; % Native bin resolution for SPAD is 4 ps
    c              = 3e8;    % Speed of light (meters per second)
%    bin_resolution = 0.006;
%    c = 1;
    width = wall_size / 2;
    if ~exist('crop', 'var') % bin index to crop measurements after aligning
        crop = 1024;          % so that direct component is at t=0
    end
...
%     meas = meas(:, :, 1:512);
    ...
    elseif (alg == 3) % phasor fields
        ...
        sampling_coeff = 3; % scale the size of the virtual wavelength (usually 2, optionally 3 for noisy scenes)
        virtual_wavelength = sampling_coeff * (s_lamda_limit * 2); % virtual wavelength in units of cm
        cycles = 4; % number of wave cycles in the wavelet, typically 4-6
